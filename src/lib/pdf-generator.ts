...
export async function generatePdf(
  ...
  try {
    ...
    autoTable(doc,{
      startY: finalY + 20,
      head: [['Date', 'Duration', 'Distance', 'Gross', 'Gas', 'Tolls', 'Food', 'Vehicle']],
      body: tableBody,
      theme: 'grid',
      headStyles: {
        fontSize: 8,
      ...
      didParseCell: (data) => {
        // Handle colspan by removing columns for the spanned row
        if (data.cell.raw && typeof data.cell.raw === 'object' && (data.cell.raw as any).colSpan) {
            data.cell.styles.halign = 'center';
            for(let i=1; i < (data.cell.raw as any).colSpan; i++) {
                if(data.row.cells[data.column.index+i]) {
                   data.row.cells[data.column.index+i].cellWidth = 0;
                }
            }
        }
      }
